...
// ---------- 1. Real v4 save migrates: progress intact, quality added, theme moved ----------
{
  ...
  report(
    'migrate: avatar roster removed (v10 Hero)',
    migrated.state.equipped.avatar === undefined && !migrated.state.owned.includes('ninja') && !migrated.state.owned.includes('cursor'),
  )
  report('migrate: hero migrated to new shape', typeof migrated.state.hero?.aura?.style === 'string' && 'accessory' in migrated.state.hero)
...
// ---------- 2. t3-ciw playable end-to-end in webgl tier ----------
{
  ...
  await page.keyboard.press('Escape') // exact-text goal completes only after leaving insert
  await page.waitForTimeout(600)
  const perfect = await page.locator('text=PERFECT!').isVisible().catch(() => false)
  report('t3-ciw: solved at 9 = par 9 → PERFECT', perfect)
...
// ---------- 2b. t3-tag-change: lazy lang-html chunk + cit in a real browser ----------
{
  ...
  await page.keyboard.press('Escape') // exact-text goal completes only after leaving insert
  await page.waitForTimeout(600)
  report('t3-tag-change: cit solves at 11 ≤ par 12 → PERFECT', await page.locator('text=PERFECT!').isVisible().catch(() => false))
